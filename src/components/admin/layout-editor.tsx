...
export function LayoutEditor() {
  ...
  async function loadSettings() {
    try {
      // Primeiro, verifica se existem configurações para cada seção
  ...
  const getComponentSettings = (componentId: string) => {
    return settings.find((s) => s.component_id === componentId)?.settings || {};
  };
            ...
            <div className="space-y-6">
              ...
              <Card>
                <CardContent className="p-6 space-y-4">
                  ...
                  <Separator className="my-4" />
                  <ImageUpload
                    value={getComponentSettings(selectedSection)?.images?.main}
                    onChange={(url) =>
                      updateSettings(selectedSection, {
                        ...getComponentSettings(selectedSection),
                        images: {
        ...
        <div className="rounded-lg border bg-background p-4 h-[calc(100vh-8rem)] overflow-auto">
          <div
            style={{
              ...
              color: getComponentSettings(selectedSection)?.colors?.text,
              padding: "1rem",
              borderRadius: "0.5rem",
            }}
          >
